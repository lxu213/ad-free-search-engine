## Code from cc-pyspark repo for processing .WARC files  
## SOURCE: https://github.com/commoncrawl/cc-pyspark
...
class CCSparkJob:
    ...
    def run_job(self, sc, sqlc):
        # take input files (.gz) and convert to an RDD of strings
        input_data = sc.textFile(self.args.input,
                                 minPartitions=self.args.num_input_partitions)
        # apply method mapPartitionsWithIndex to RDD, input_data: maps function 
        # process_warcs across partition while keeping index
        # output is not a new RDD, now reduceByKey function across it   
        # reduceByKey returns pipelinedRDD
